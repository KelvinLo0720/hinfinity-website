...
export const applicantSchema = z.object({
  ...
  yearOfStudy: z.string().trim().min(1).max(80),
...
export const applicationSchema = z.object({
  ...
  if (value.applicationType === "individual" && value.applicants.length !== 1) {
    context.addIssue({ code: "custom", path: ["applicants"], message: "個人申請只能有一位申請者。" });
  }
  if (value.applicationType === "team" && (value.applicants.length < 2 || value.applicants.length > 4)) {
    context.addIssue({ code: "custom", path: ["applicants"], message: "團隊申請必須有 2 至 4 位申請者。" });
  }
});
