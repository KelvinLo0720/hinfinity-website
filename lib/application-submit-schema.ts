...
export const applicantSchema = z.object({
  ...
  institution: z
    ...
    .refine(
      (value) =>
        value !== OTHER_INSTITUTION_OPTION,
...
export const applicationSchema = z
  ...
  .superRefine((value, context) => {
    ...
    if (
      value.applicationType ===
        "team" &&
      (value.applicants.length < 2 ||
        value.applicants.length > 4)
    ) {
      context.addIssue({
        code: "custom",
        path: ["applicants"],
    ...
    if (
      value.interviewTimePreference.includes(
        "Flexible"
      ) &&
      value.interviewTimePreference
        .length > 1
    ) {
      context.addIssue({
        code: "custom",
