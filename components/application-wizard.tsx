...
function normaliseLoadedData(value: unknown): FormData {
  ...
  const applicants =
    Array.isArray(raw.applicants) && raw.applicants.length
      ? raw.applicants
          .slice(0, applicationConfig.maxTeamSize)
          .map((applicant) => {
            const merged = {
              ...emptyApplicant(),
              ...(applicant || {}),
            ...
            // If an old local draft contains a value no longer in the allow-list,
            // clear it so the applicant must make a valid selection.
            if (
              !institutionOptions.includes(
                merged.institution as (typeof institutionOptions)[number]
              )
            ) {
              merged.institution = "";
            }
            ...
            if (
              !yearOfStudyOptions.includes(
                merged.yearOfStudy as (typeof yearOfStudyOptions)[number]
              )
            ) {
              merged.yearOfStudy = "";
            }
  ...
  return {
    ...
    ...raw,
    applicationType:
      raw.applicationType === "team" ? "team" : "individual",
    applicants
  };
}
...
export function ApplicationWizard({
  ...
  useEffect(() => {
    ...
    const storedEmail =
      localStorage.getItem(`hinfinity:${token}:email`) || "";
  ...
  function removeMember(index: number) {
    ...
    setData((previous) => ({
      ...previous,
      applicants: previous.applicants.filter(
        (_, i) => i !== index
      )
    }));
  ...
  function selectCv(
    ...
    setStatus(
      t(
        "CV 已準備好，會喺正式提交時安全上載到 Notion。",
        "CV ready. It will be securely uploaded to Notion when you submit."
      )
    );
  }

  function applicantIsComplete(
    applicant: Applicant
  ) {
    const validInstitution =
      institutionOptions.includes(
        applicant.institution as (typeof institutionOptions)[number]
      );

    const validYear =
      yearOfStudyOptions.includes(
        applicant.yearOfStudy as (typeof yearOfStudyOptions)[number]
      );
  ...
  function validateCurrentStep() {
    ...
    if (step === 1) {
      ...
      if (
        ...
        setStatus(
          t(
            "請完成所有申請者嘅必填基本資料，並從指定名單選擇院校及就讀年級。",
            "Please complete the required details for every applicant and select an institution and year of study from the approved lists."
          )
        );
  ...
  async function submitApplication() {
    ...
    if (
      ...
      setStatus(
        t(
          "申請者資料包含無效院校或就讀年級，請返回重新選擇。",
          "Applicant details contain an invalid institution or year of study. Please go back and select again."
        )
      );
    ...
    setStatus(
      t(
        "正在安全提交到 H Infinity…",
        "Submitting securely to H Infinity…"
      )
    );
                    ...
                    >
                      ...
                      <div className="field">
                        ...
                        <small>
                          {t(
                            "只接受以上名單內的香港大專及大學院校。",
                            "Only institutions on the approved Hong Kong post-secondary institution list can be selected."
                          )}
                        </small>
                      </div>
